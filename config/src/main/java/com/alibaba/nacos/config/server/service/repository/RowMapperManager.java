...
public final class RowMapperManager {
    ...
    public static final RowMapper<Music> MUSIC_ROW_MAPPER = new MusicRowMapper();
    ...
    public static final class MusicRowMapper implements RowMapper<Music> {

        @Override
        public Music mapRow(ResultSet rs, int rowNum) throws SQLException {
            Music mic = new Music();
            // 数据库结果集映射到 Music实例对象
            mic.setName(rs.getString("name"));
            mic.setSongCount(rs.getInt("songCount"));
            return mic;
        }
    }
